...
function Write-Result([string]$Status,[System.Collections.IDictionary]$Counts,[System.Collections.IDictionary]$Fingerprints,[string[]]$Blockers) {
    [ordered]@{status=$Status;identity=[ordered]@{localdb=($Status -eq 'READY_FOR_DISPOSABLE_REHEARSAL');databaseClass='TASK_364_FIXED_DISPOSABLE';schema=$ExpectedSchema};counts=$Counts;schemaFingerprints=$Fingerprints;blockers=@($Blockers);productionAuthorized=$false} |
...
try {
    ...
    if($baseline.reportType -cne 'JemNexusProductionSchemaBaseline' -or $baseline.database -cne 'jemnexusb_prod' -or $baseline.schema -cne 'jemnexusb_api' -or [int]$baseline.migrationCount -ne 22 -or [string]::IsNullOrWhiteSpace([string]$baseline.evidenceCapturedUtc)){throw 'PRODUCTION_BASELINE_INVALID'}
    ...
    $migrations=Invoke-ReadOnly $connection "SELECT MigrationId FROM [dbo].[__EFMigrationsHistory] ORDER BY MigrationId"
    ...
    $tables=Invoke-ReadOnly $connection "SELECT t.name TableName FROM sys.tables t JOIN sys.schemas s ON s.schema_id=t.schema_id WHERE s.name='dbo' AND t.name IN ('$($ImportTables -join "','")') ORDER BY t.name"
    if($tables.Rows.Count -ne 17){$blockers.Add('IMPORT_TABLE_SET_MISMATCH')}
    $metadataQueries=[ordered]@{
      columns=@("SELECT t.name TableName,c.column_id Ordinal,c.name ColumnName,ty.name TypeName,c.max_length MaxLength,c.precision Precision,c.scale Scale,c.is_nullable Nullable,c.collation_name Collation FROM sys.tables t JOIN sys.schemas s ON s.schema_id=t.schema_id JOIN sys.columns c ON c.object_id=t.object_id JOIN sys.types ty ON ty.user_type_id=c.user_type_id WHERE s.name='dbo' AND t.name IN ('$($ImportTables -join "','")') ORDER BY t.name,c.column_id",@('TableName','Ordinal','ColumnName','TypeName','MaxLength','Precision','Scale','Nullable','Collation'))
      primaryKeys=@("SELECT t.name TableName,k.name KeyName,ic.key_ordinal Ordinal,c.name ColumnName FROM sys.key_constraints k JOIN sys.tables t ON t.object_id=k.parent_object_id JOIN sys.schemas s ON s.schema_id=t.schema_id JOIN sys.index_columns ic ON ic.object_id=t.object_id AND ic.index_id=k.unique_index_id JOIN sys.columns c ON c.object_id=t.object_id AND c.column_id=ic.column_id WHERE s.name='dbo' AND k.type='PK' AND t.name IN ('$($ImportTables -join "','")') ORDER BY t.name,ic.key_ordinal",@('TableName','KeyName','Ordinal','ColumnName'))
      foreignKeys=@("SELECT pt.name TableName,fk.name ForeignKey,pc.name ColumnName,rt.name ReferencedTable,rc.name ReferencedColumn,fk.delete_referential_action_desc DeleteAction,fk.is_disabled IsDisabled,fk.is_not_trusted IsNotTrusted FROM sys.foreign_keys fk JOIN sys.tables pt ON pt.object_id=fk.parent_object_id JOIN sys.schemas s ON s.schema_id=pt.schema_id JOIN sys.foreign_key_columns fkc ON fkc.constraint_object_id=fk.object_id JOIN sys.columns pc ON pc.object_id=fkc.parent_object_id AND pc.column_id=fkc.parent_column_id JOIN sys.tables rt ON rt.object_id=fk.referenced_object_id JOIN sys.columns rc ON rc.object_id=fkc.referenced_object_id AND rc.column_id=fkc.referenced_column_id WHERE s.name='dbo' AND pt.name IN ('$($ImportTables -join "','")') ORDER BY pt.name,fk.name,fkc.constraint_column_id",@('TableName','ForeignKey','ColumnName','ReferencedTable','ReferencedColumn','DeleteAction','IsDisabled','IsNotTrusted'))
      indexes=@("SELECT t.name TableName,i.name IndexName,i.is_unique IsUnique,i.filter_definition FilterDefinition,ic.key_ordinal Ordinal,c.name ColumnName FROM sys.indexes i JOIN sys.tables t ON t.object_id=i.object_id JOIN sys.schemas s ON s.schema_id=t.schema_id JOIN sys.index_columns ic ON ic.object_id=i.object_id AND ic.index_id=i.index_id JOIN sys.columns c ON c.object_id=ic.object_id AND c.column_id=ic.column_id WHERE s.name='dbo' AND i.is_hypothetical=0 AND i.name IS NOT NULL AND t.name IN ('$($ImportTables -join "','")') ORDER BY t.name,i.name,ic.key_ordinal,ic.index_column_id",@('TableName','IndexName','IsUnique','FilterDefinition','Ordinal','ColumnName'))
      checks=@("SELECT t.name TableName,cc.name CheckName,cc.definition Definition,cc.is_disabled IsDisabled,cc.is_not_trusted IsNotTrusted FROM sys.check_constraints cc JOIN sys.tables t ON t.object_id=cc.parent_object_id JOIN sys.schemas s ON s.schema_id=t.schema_id WHERE s.name='dbo' AND t.name IN ('$($ImportTables -join "','")') ORDER BY t.name,cc.name",@('TableName','CheckName','Definition','IsDisabled','IsNotTrusted'))
      identities=@("SELECT t.name TableName,ic.name ColumnName,CONVERT(nvarchar(40),ic.seed_value) SeedValue,CONVERT(nvarchar(40),ic.increment_value) IncrementValue FROM sys.identity_columns ic JOIN sys.tables t ON t.object_id=ic.object_id JOIN sys.schemas s ON s.schema_id=t.schema_id WHERE s.name='dbo' AND t.name IN ('$($ImportTables -join "','")') ORDER BY t.name,ic.name",@('TableName','ColumnName','SeedValue','IncrementValue'))
      sequence=@("SELECT s.name SchemaName,q.name SequenceName,CONVERT(nvarchar(40),q.start_value) StartValue,CONVERT(nvarchar(40),q.increment) IncrementValue FROM sys.sequences q JOIN sys.schemas s ON s.schema_id=q.schema_id WHERE s.name='dbo' AND q.name='$RequiredSequence'",@('SchemaName','SequenceName','StartValue','IncrementValue'))
    }
    ...
    $permission=Invoke-ReadOnly $connection "SELECT HAS_PERMS_BY_NAME(DB_NAME(),'DATABASE','CONNECT') CanConnect,HAS_PERMS_BY_NAME('dbo','SCHEMA','SELECT') CanSelect,HAS_PERMS_BY_NAME('dbo','SCHEMA','INSERT') CanInsert,HAS_PERMS_BY_NAME('dbo','SCHEMA','UPDATE') CanUpdate,HAS_PERMS_BY_NAME('dbo','SCHEMA','DELETE') CanDelete,HAS_PERMS_BY_NAME('dbo','SCHEMA','ALTER') CanAlter"
    ...
    $folios=Invoke-ReadOnly $connection "SELECT (SELECT COUNT_BIG(*) FROM dbo.CommercialQuoteFolioCounters) CounterRows,(SELECT COUNT_BIG(*) FROM dbo.CommercialQuotes q LEFT JOIN dbo.CommercialQuoteFolioCounters fc ON fc.Year=q.FolioYear WHERE q.FolioYear IS NOT NULL AND q.FolioSequenceNumber IS NOT NULL AND (fc.LastNumber IS NULL OR fc.LastNumber<q.FolioSequenceNumber)) InvalidFolios"
