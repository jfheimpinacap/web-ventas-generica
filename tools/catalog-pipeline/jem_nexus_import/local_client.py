"""Pure validation and consumption of injected local JSON GET results."""
...
class LocalJemJsonReader:
    def __init__(self,base_url,transport=None,requires_auth=True,timeout=10,max_bytes=4_000_000):
        self.base_url=validate_base_url(base_url); self.transport=transport; self.timeout=timeout; self.max_bytes=max_bytes
        if transport is None: raise LocalReadError("LOCAL_TRANSPORT_MISSING","an explicit local GET transport is required")
