...
def _exercise_generated(test,case):
    ...
    if category=="authorization":
        ...
        elif variant=="schemas": test.assertEqual(len(list((root/"schemas/v1").glob("*.schema.json"))),82)
        else: test.assertEqual(len(list((root/"fixtures/valid").glob("local-*.json"))),8)
