...
def replay(snapshot_manifest:dict,root:Path,output_dir:Path,adapters)->dict:
    ...
    _jsonl(output_dir/"categories.jsonl",categories); _jsonl(output_dir/"product-candidates.jsonl",candidates); _jsonl(output_dir/"frontier.jsonl",frontier); atomic_write(output_dir/"discovery-manifest.json",canonical_bytes(payload)); atomic_write(output_dir/"discovery-report.txt",_report(payload).encode("utf-8"))
    return payload
