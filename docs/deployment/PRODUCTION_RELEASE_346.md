...
La herramienta divide el SQL EF solo por sus separadores `GO` y encapsula **cada lote EF no vacío** en su propia guarda server-side. Antes de ejecutar el cuerpo, la guarda exige: (1) que exista el centinela temporal; (2) que `XACT_STATE() = 1`, es decir, que la transacción exterior siga activa y confirmable; (3) que el marcador de fallo esté limpio; y (4) que el ordinal sea exactamente el del lote anterior. El cuerpo conserva las instrucciones EF; el ordinal avanza únicamente después de que el cuerpo entero termina. El lote siguiente exige ese avance exacto.

Los conteos se derivan del SQL EF generado y se validan antes de publicar: para el SQL vigente son **41 lotes protegidos, cuatro directos y 37 dinamicos**. Los cuatro directos son exactamente dos cuerpos puros `BEGIN TRANSACTION;` y dos cuerpos puros `COMMIT;`. Conservan su posición, guarda, `TRY/CATCH`, comprobación posterior de `XACT_STATE()` y avance ordinal, pero ejecutan la sentencia original directamente. Los otros 37 cuerpos siguen dentro de `sp_executesql`, de modo que sus errores de compilación son capturables. La clasificación es cerrada: rechaza lotes mixtos y variantes transaccionales inesperadas, y la validación impide que `BEGIN TRANSACTION`, `COMMIT`, `COMMIT TRANSACTION` o `ROLLBACK TRANSACTION` entren en un cuerpo dinámico.
...
El SQL de EF puede emitir sus propios `BEGIN TRANSACTION` y `COMMIT`. En SQL Server son transacciones anidadas por contador: esos `COMMIT` internos reducen `@@TRANCOUNT`, pero no hacen persistentes los cambios mientras la transacción exterior siga abierta. Deben ejecutarse directamente porque `sp_executesql` no puede regresar con un `@@TRANCOUNT` distinto del que tenía al entrar: envolver el primer `BEGIN TRANSACTION;` elevó el contador de 1 a 2 dentro de `EXECUTE` y produjo **Msg 266**. La secuencia correcta es transacción exterior 1, `BEGIN` EF directo 2, `COMMIT` EF directo 1, repetición del segundo par, validación y `COMMIT` exterior directo hasta 0.

El postflight tiene su propia guarda y exige el ordinal final exacto de todos los lotes EF. El incidente produjo después **Msg 207** porque una referencia estática a `IssuedById` se compiló en el nivel exterior antes de que pudiera actuar la guarda, cuando la migración había quedado bloqueada. Ahora todas las referencias a `AppUserPermissions`, `IssuedById`, su índice y su FK se compilan en un `sp_executesql` inferior únicamente después de comprobar centinela, transacción confirmable, `ExecutionFailed = 0` y ordinal final.

Esa validación dinámica no contiene control transaccional. Exige ambas migraciones una vez, tabla de permisos, exactamente 29 permisos por seller calculados mediante joins/subconsultas (sin IDs de usuario hardcodeados), ausencia de `users.manage`, columna obligatoria, índice, FK `NO_ACTION`, cero cotizaciones con emisor nulo y conteo de `QuoteRequests` igual al valor leído desde el centinela. Devuelve los cinco conteos mediante parámetros `OUTPUT`. Tras completarla se vuelve a exigir `XACT_STATE() = 1`; solo entonces el nivel exterior ejecuta directamente `COMMIT TRANSACTION`. La fila final usa exclusivamente las variables calculadas, emite una sola vez `POSTFLIGHT_OK` y después elimina la tabla temporal.

### Incidente productivo y releases invalidados

La comprobación independiente posterior confirmó un **rollback limpio**: `@@TRANCOUNT = 0` y `XACT_STATE() = 0` antes y después de la recuperación; seguían existiendo 20 filas de historial y cero migraciones objetivo; no existían `AppUserPermissions`, `IssuedById`, su índice ni su FK; había cero cotizaciones comerciales y ocho solicitudes. Por tanto no hubo migración parcial, transacción abierta, reparación de datos necesaria ni pérdida de información.

Los releases `f6f4f2027ae8` y `490cdcf41dfb` quedan invalidados. Ninguno de sus ZIP ni SQL debe desplegarse o reutilizarse. Debe generarse un release completo nuevo desde el HEAD limpio posterior al próximo merge. Antes de regresar a producción será obligatoria una validación dinámica en Windows y SQL Server sobre una base local desechable equivalente, incluyendo la secuencia real de transacciones y el caso bloqueado sin schema nuevo.
...
Los dos releases invalidados identificados arriba quedan reemplazados por esta corrección y **no deben aplicarse en producción**. Después del merge, elimine de forma explícita sus `OutputRoot`, vuelva a comprobar que el checkout está en el nuevo `HEAD` de `main` limpio y genere un release completo nuevo desde ese commit. No reutilice sus SQL, ZIP, manifiestos ni checksums.
