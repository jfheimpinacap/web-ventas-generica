...
En un Windows controlado quedan obligatorias la nueva validación del parser y la ejecución real de `PlanOnly`, seguida de `InventoryLocal` contra la LocalDB confirmada. Esta repetición de `InventoryLocal` debe confirmar que las consultas de metadata e índices reciben `@schema` en el mismo `SqlCommand` antes de `SqlDataAdapter.Fill`, que cada consulta conserva un `System.Data.DataTable` con cero, una o varias filas y que la lista tipada de fallos de forma conserva su tipo con cero, uno o varios elementos antes de consultar `Count`. También debe confirmar que el recorrido antirreparse trata el archivo inicial como `System.IO.FileInfo` (su padre se obtiene mediante `Directory`) y sus ancestros como `System.IO.DirectoryInfo`. Estas comprobaciones regresan los fallos observados `Must declare the scalar variable "@schema"` y `PropertyNotFoundStrict` para `Parent` y para `Count`; este último procedía de la enumeración de una `List[string]`, convertida por PowerShell en un `System.String` cuando contenía un solo fallo. En particular, confirme que un nombre efectivo dinámico pasa gracias a `IsLocalDB = 1`, y que base incorrecta o `IsLocalDB` no inequívoco abortan sin informe. Verifique antes y después que no se creó un informe ante ningún fallo, que `PlanOnly` no creó el directorio y que `git status --short` sigue limpio. Compare luego con un snapshot sanitizado revisado. No pruebe conexión productiva: la herramienta no la admite.
