...
export function CustomerSearch({ disabled, selectedId, onSelect, onUnlink }: { disabled: boolean; selectedId: number | null; onSelect: (value: CustomerProfile) => void; onUnlink: () => void }) {
  ...
  return <section className="commercial-section"><div className="commercial-section__heading"><h2>Buscar cliente</h2>{selectedId ? <button className="button button--ghost" type="button" onClick={onUnlink} disabled={disabled}>Desvincular perfil</button> : null}</div>
    <label htmlFor="customer-search">Buscar por razón social o RUT</label><input id="customer-search" placeholder="Buscar por razón social o RUT" value={query} maxLength={200} disabled={disabled} onChange={e => setQuery(e.target.value)} aria-controls="customer-results" />
