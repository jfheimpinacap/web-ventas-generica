...
export function UserForm({ initialFields, passwordRequired, mode, currentSession = false, onSubmit, onCancel }: Props) {
  ...
  const field = (key: keyof Errors, label: string, type = 'text') => <div className="admin-user-field"><label htmlFor={key}>{label}</label><input ref={(node) => { refs.current[key] = node }} id={key} type={type} autoComplete={type === 'password' ? 'new-password' : undefined} maxLength={key === 'fullName' ? 180 : undefined} value={String(fields[key])} onChange={(event) => setText(key, event.target.value)} aria-invalid={Boolean(errors[key])} />{errors[key] ? <span className="admin-field-error">{errors[key]}</span> : null}</div>
  return <form className="admin-user-form-page" onSubmit={(event) => void submit(event)} noValidate>
    <section className="admin-block admin-access-block"><h2>Datos de acceso</h2><div aria-live="assertive">{generalError ? <p className="ui-note ui-note--error">{generalError}</p> : null}</div><div className="admin-user-form-grid">{field('username', 'Nombre de usuario')}{field('email', 'Correo electrónico (opcional)', 'email')}{field('password', mode === 'edit' ? 'Nueva contraseña (opcional)' : 'Contraseña nueva', 'password')}{field('confirmation', 'Confirmar contraseña', 'password')}<div className="admin-user-field"><label htmlFor="role">Rol de acceso</label><select id="role" value={fields.role} disabled={currentSession} onChange={(event) => setFields((current) => ({ ...current, role: event.target.value as ManagedRole }))}><option value="seller">Vendedor</option><option value="support_admin">Superadministrador</option></select></div>{field('fullName', 'Nombre visible (opcional)')}</div>{currentSession ? <p className="ui-note">No puedes desactivar ni cambiar el rol de tu sesión actual.</p> : null}</section>
