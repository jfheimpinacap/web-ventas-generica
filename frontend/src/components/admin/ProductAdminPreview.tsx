...
export function ProductAdminPreview({ values, categories, imageUrl, imageAlt }: Props) {
  ...
  const serviceSummary = values.description.trim().slice(0, 280)
  return <article className="product-card admin-product-preview-card">
    ...
    <div className="product-card__content">
      ...
      {isService ? <p className="admin-product-preview__summary">{serviceSummary || 'Agrega una descripción para ver el adelanto del servicio.'}</p> : null}
