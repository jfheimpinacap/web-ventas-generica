...
import { isSeller, isSupportAdmin } from '../../services/authApi'
import { listManagedUsers, type ManagedUser } from '../../services/adminUsersApi'
...
export function CommercialQuoteEditorPage() {
  ...
  const supportAdmin = isSupportAdmin(currentUser ?? undefined)
  ...
  const [eligibleSellers, setEligibleSellers] = useState<ManagedUser[]>([]); const [selectedSellerId, setSelectedSellerId] = useState<number | null>(null); const [sellersLoading, setSellersLoading] = useState(false)
  const [loading, setLoading] = useState(Boolean(routeId)); const [saving, setSaving] = useState(false); const [dirty, setDirty] = useState(false); const [message, setMessage] = useState(''); const [error, setError] = useState(''); const [rutReviewed, setRutReviewed] = useState(false); const [showIssueDialog, setShowIssueDialog] = useState(false); const firstError = useRef<HTMLDivElement>(null); const rutInput = useRef<HTMLInputElement>(null); const issueButton = useRef<HTMLButtonElement>(null); const readonly = Boolean(routeId) || (!isSeller(currentUser ?? undefined) && !supportAdmin)
  ...
  const selectedSeller = eligibleSellers.find(user => user.id === selectedSellerId)
  const seller = persisted ? { name: persisted.responsibleSellerName, code: persisted.responsibleSellerCode, email: persisted.responsibleSellerEmail, phone: persisted.responsibleSellerPhone } : supportAdmin ? { name: selectedSeller?.full_name?.trim() || selectedSeller?.username, code: selectedSeller?.seller_code, email: selectedSeller?.email, phone: selectedSeller?.phone } : { name: currentSellerName, code: currentUser?.seller_code, email: currentUser?.email, phone: currentUser?.phone }
  ...
  useEffect(() => {
    if (!supportAdmin || routeId) return
    const controller = new AbortController(); setSellersLoading(true)
    listManagedUsers({ role: 'seller', is_active: true }, controller.signal)
      .then(users => setEligibleSellers(users.filter(user => user.role === 'seller' && user.is_active && Boolean(user.seller_code?.trim()))))
      .catch(error => { if (!controller.signal.aborted) setError(getSafeApiErrorMessage(error, 'No se pudieron cargar los vendedores responsables.')) })
      .finally(() => { if (!controller.signal.aborted) setSellersLoading(false) })
    return () => controller.abort()
  }, [routeId, supportAdmin])
  ...
  const selectCustomer = (p: CustomerProfile) => { setCustomer({ customer_profile_id:p.id, customer_business_name:p.businessName, customer_rut:formatChileanRutInput(p.rut), customer_business_activity:p.businessActivity, customer_address:p.address, customer_phone:p.phone, customer_city_or_commune:p.cityOrCommune, customer_contact_name:p.contactName, customer_email:p.email ?? '' }); setRutReviewed(false); touch() }
  ...
  const validate = (requireItems: boolean) => { setRutReviewed(true); const errors: string[] = []; if (requireItems && supportAdmin && selectedSellerId === null) errors.push('Seleccione un vendedor responsable.'); customerFields.filter(f=>f[3] && f[0] !== 'customer_rut').forEach(([key,label]) => { if (!String(customer[key]).trim()) errors.push(`${label} es obligatorio.`) }); const normalizedRut = normalizeChileanRut(customer.customer_rut); if (customer.customer_email && !/^\S+@\S+\.\S+$/.test(customer.customer_email)) errors.push('Ingrese un correo válido.'); if (!/\d/.test(customer.customer_phone)) errors.push('El teléfono debe contener al menos un dígito.'); const filled = items.filter(i=>!isEmptyItem(i)); for (const item of filled) { const discount = Number(item.discount_percent || 0); if (!item.product_name || !Number.isInteger(Number(item.quantity)) || Number(item.quantity)<1 || !Number.isFinite(Number(item.unit_net_amount)) || Number(item.unit_net_amount)<=0 || !Number.isFinite(discount) || discount<0 || discount>100 || (item.discount_percent !== '' && !/^\d+(\.\d{0,2})?$/.test(item.discount_percent))) errors.push('Corrija o elimine las filas de productos incompletas.') } if (requireItems && !filled.length) errors.push('Agregue al menos un ítem para emitir.'); setError(errors[0] ?? ''); if (!normalizedRut) rutInput.current?.focus(); else if (errors.length) firstError.current?.focus(); return Boolean(normalizedRut) && !errors.length }
  const payload = () => ({ ...customer, customer_rut: normalizeChileanRut(customer.customer_rut)!, currency, sale_condition: condition, validity_days: validityDays, detailed_description: description || undefined, ...(supportAdmin && selectedSellerId !== null ? { seller_user_id: selectedSellerId } : {}), items: items.filter(i=>!isEmptyItem(i)).map(i => ({ source: i.source as 'Catalog'|'FreeText', ...(i.product_id ? { product_id:i.product_id } : {}), product_name:i.product_name, ...(i.brand_name ? {brand_name:i.brand_name}:{}), ...(i.model_name ? {model_name:i.model_name}:{}), quantity:Number(i.quantity), unit_net_amount:Number(i.unit_net_amount), ...(i.discount_percent ? {discount_percent:Math.min(100,Math.max(0,Number(i.discount_percent)))}:{}) })) })
  ...
  const issue = async () => { if (saving || readonly) return; const issuePayload = payload(); const payloadFingerprint = JSON.stringify(issuePayload); if (!pendingIssue.current || pendingIssue.current.payloadFingerprint !== payloadFingerprint) pendingIssue.current = { payloadFingerprint, idempotencyKey: crypto.randomUUID() }; const idempotencyKey = pendingIssue.current.idempotencyKey; setShowIssueDialog(false); setSaving(true); setError(''); try { const issued = await issueCommercialQuote(issuePayload, idempotencyKey); pendingIssue.current = null; apply(issued); setIssuedHere(true); setMessage(`Cotización emitida con folio ${issued.folio}.`); toast.success(ADMIN_TOASTS.commercialQuote.issue.success); navigate(`/admin/cotizaciones/${issued.id}/editar`, { replace:true, state: { issuedQuotePdfPreview: { quoteId: issued.id, folio: issued.folio } } }) } catch(e) { toast.error(ADMIN_TOASTS.commercialQuote.issue.error); setError(getSafeApiErrorMessage(e,'No se pudo emitir la cotización. Los datos del formulario se conservaron.')) } finally { setSaving(false) } }
  ...
  return <AdminLayout><AdminPageHeader title={id ? 'Ver cotización' : 'Crear cotización'} /><div ref={firstError} tabIndex={-1}>{error ? <p className="ui-note ui-note--error" role="alert">{error}</p>:null}{downloadError ? <p className="ui-note ui-note--error" role="alert">{downloadError}</p>:null}{message ? <p className="ui-note ui-note--success">{message}</p>:null}</div>
    <div className="commercial-editor-grid">
      <div className="commercial-editor-main">
      <section className="commercial-section commercial-customer-panel">
        ...
        <button className="btn btn--accent commercial-primary-action" type="button" disabled={readonly||saving} onClick={async()=>{if(!validate(false))return;setSaving(true);try{const body={business_name:customer.customer_business_name,rut:normalizeChileanRut(customer.customer_rut)!,business_activity:customer.customer_business_activity,address:customer.customer_address,phone:customer.customer_phone,city_or_commune:customer.customer_city_or_commune,contact_name:customer.customer_contact_name,email:customer.customer_email||null};const saved=await saveCustomer(body,customer.customer_profile_id??undefined);selectCustomer(saved);setMessage('Cliente guardado correctamente.');toast.success(customer.customer_profile_id ? ADMIN_TOASTS.customer.update.success : ADMIN_TOASTS.customer.create.success)}catch(e){toast.error(customer.customer_profile_id ? ADMIN_TOASTS.customer.update.error : ADMIN_TOASTS.customer.create.error);setError(getSafeApiErrorMessage(e,'No se pudo guardar el cliente. Si el RUT ya existe, búsquelo arriba.'))}finally{setSaving(false)}}}>Guardar cliente</button>
      </section>
        <section className="commercial-section">
          <h2>Datos comerciales y productos a cotizar</h2>
          {supportAdmin && !persisted ? <div className="commercial-customer-validation">
            <label className="commercial-meta-field">Vendedor responsable *
              <select aria-label="Vendedor responsable" required value={selectedSellerId ?? ''} disabled={saving || sellersLoading || eligibleSellers.length === 0} onChange={event=>{setSelectedSellerId(event.target.value ? Number(event.target.value) : null);touch()}}>
                <option value="">Seleccione un vendedor</option>
                {eligibleSellers.map(user=><option key={user.id} value={user.id}>{user.full_name?.trim() || user.username} — {user.seller_code}</option>)}
              </select>
            </label>
            {!sellersLoading && eligibleSellers.length === 0 ? <p className="ui-note ui-note--error" role="alert">No existen vendedores activos elegibles para emitir la cotización.</p> : null}
          </div> : null}
        ...
        <div className="commercial-lower-row">
          ...
          <div className="commercial-summary-column"><section className="commercial-section quote-values"><h2>Valor final</h2><dl><div><dt>Neto</dt><dd>{money(totals.net,currency)}</dd></div><div><dt>IVA 19%</dt><dd>{money(totals.tax,currency)}</dd></div><div><dt>Total</dt><dd>{money(totals.total,currency)}</dd></div></dl><small>Vista previa; el cálculo definitivo lo realiza el servidor.</small></section><div className="commercial-actions"><button className="btn btn--ghost" type="button" onClick={goBack}>Volver</button>{!readonly || issuedHere ? <button ref={issueButton} className="btn btn--accent commercial-primary-action" type="button" disabled={saving || (supportAdmin && (sellersLoading || eligibleSellers.length === 0 || selectedSellerId === null))} aria-disabled={readonly || undefined} aria-busy={saving || undefined} onClick={requestIssue}>{saving ? 'Emitiendo…' : 'Emitir cotización'}</button> : null}{persisted?.status === 'Issued' && persisted.folio?.trim() && Number.isInteger(persisted.id) && persisted.id > 0 ? <button className="btn btn--secondary" type="button" disabled={isDownloading(persisted.id)} aria-busy={isDownloading(persisted.id) || undefined} aria-label={`${isDownloading(persisted.id) ? 'Descargando' : 'Descargar PDF de la cotización'} ${persisted.folio}`} onClick={() => void downloadPdf(persisted.id, persisted.folio)}>{isDownloading(persisted.id) ? 'Descargando…' : 'Descargar PDF'}</button> : null}</div></div>
        </div>
      </div>
    </div>
