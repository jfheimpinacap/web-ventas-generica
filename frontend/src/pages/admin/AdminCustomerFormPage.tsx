...
export function AdminCustomerFormPage() {
  const { id } = useParams(); const [params] = useSearchParams(); const navigate = useNavigate(); const toast = useToast()
  ...
  const save = async (values: Parameters<typeof createCustomer>[0]) => { if (submitting) return; setSubmitting(true); setError(''); try { if (editing) await updateCustomer(customerId, values); else await createCustomer(values); toast.success(editing ? ADMIN_TOASTS.customer.update.success : ADMIN_TOASTS.customer.create.success); navigate(`${returnTo}${returnTo.includes('?') ? '&' : '?'}result=${editing ? 'updated' : 'created'}`, { replace: true }) } catch (caught) { toast.error(editing ? ADMIN_TOASTS.customer.update.error : ADMIN_TOASTS.customer.create.error); setError(caught instanceof ApiError && caught.status === 409 ? 'Ya existe un cliente con ese RUT. Puede ser necesario reactivarlo desde el listado de clientes.' : getSafeApiErrorMessage(caught, 'No se pudo guardar el cliente.')) } finally { setSubmitting(false) } }
