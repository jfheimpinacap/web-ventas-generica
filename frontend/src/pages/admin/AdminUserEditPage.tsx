...
export function AdminUserEditPage() {
  const { userId } = useParams<{ userId: string }>(); const navigate = useNavigate(); const session = useAdminUser(); const id = userId && /^\d+$/.test(userId) ? Number(userId) : null
  ...
  const submit = async (payload: ManagedUserWrite, clearSensitive: () => void) => { try { await updateManagedUser(user.id, payload) } catch (reason) { if (reason instanceof ApiError && reason.status === 401) { clearSensitive(); clearSession(); navigate('/login', { replace: true }); return } throw reason } clearSensitive(); if (currentSession && payload.password) { clearSession(); navigate('/login', { replace: true, state: { notice: 'Contraseña actualizada. Ingresa nuevamente.' } }); return } navigate('/admin/usuarios', { replace: true, state: { notice: reactivating ? 'Usuario reactivado correctamente.' : 'Usuario actualizado correctamente.' } }) }
