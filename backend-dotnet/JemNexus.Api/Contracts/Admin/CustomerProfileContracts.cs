...
public sealed record CustomerProfileResponse(int Id, string BusinessName, string Rut, string BusinessActivity, string Address, string Phone, string CityOrCommune, string ContactName, string? Email, bool IsActive, DateTimeOffset CreatedAt, DateTimeOffset UpdatedAt)
{
    public static CustomerProfileResponse FromEntity(CustomerProfile value) => new(value.Id, value.BusinessName, value.Rut, value.BusinessActivity, value.Address, value.Phone, value.CityOrCommune, value.ContactName, value.Email, value.IsActive, value.CreatedAt, value.UpdatedAt);
}
