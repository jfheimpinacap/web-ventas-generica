...
public sealed class CommercialQuoteModelTests
{
    ...
    public void ModelDefinesQuoteRelationshipsAndPrecision()
    {
        ...
        Assert.Equal(nameof(CommercialQuote.Id), quote.FindPrimaryKey()!.Properties.Single().Name); Assert.True(quote.FindProperty(nameof(CommercialQuote.CustomerProfileId))!.IsNullable); Assert.False(quote.FindProperty(nameof(CommercialQuote.ResponsibleSellerId))!.IsNullable); Assert.False(quote.FindProperty(nameof(CommercialQuote.IssuedById))!.IsNullable); Assert.True(item.FindProperty(nameof(CommercialQuoteItem.ProductId))!.IsNullable);
        var userForeignKeys = quote.GetForeignKeys().Where(foreignKey => foreignKey.PrincipalEntityType.ClrType == typeof(AppUser)).ToArray();
        Assert.Equal(2, userForeignKeys.Length); Assert.All(userForeignKeys, foreignKey => Assert.Equal(DeleteBehavior.NoAction, foreignKey.DeleteBehavior));
        Assert.Contains(userForeignKeys, foreignKey => foreignKey.Properties.Single().Name == nameof(CommercialQuote.ResponsibleSellerId) && foreignKey.DependentToPrincipal?.Name == nameof(CommercialQuote.ResponsibleSeller));
        Assert.Contains(userForeignKeys, foreignKey => foreignKey.Properties.Single().Name == nameof(CommercialQuote.IssuedById) && foreignKey.DependentToPrincipal?.Name == nameof(CommercialQuote.IssuedBy));
        Assert.Contains(quote.GetIndexes(), index => index.Properties.Select(property => property.Name).SequenceEqual([nameof(CommercialQuote.IssuedById)]));
